...
class Node {
    ...
    static Node[][] getStructures(String line) {
        Node[][] structures = new Node[9][9];
        try {
            int current = 0;
            for (int i = 0; i < 9; i++) {
                if (line != null) {
                    ...
                    for (int j = 0; j < 9; j++) {
                        int number = Integer.parseInt(numbers[current]);
                        if (number > 0 && number < 10) {
                            structures[i][j] = new Node(i, j, number);
                        } else {
                            structures[i][j] = new Node(i, j);
                        }
                        current ++;
                    }
                } else {
    ...
    private static void printPuzzle(String filename, Node[][] nodes) {
        System.out.println("loading rreshtin: " + filename + "...\r\n");
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                ...
                if (nodes[i][j].domain.size() > 1) {
                    ...
                    System.out.print(nodes[i][j].printDomain2());
                }
            }
            System.out.print("\r\n");
        }
...
//        }
    ...
    //9 box gjithsej 0-8
    //sipas nr te kolones dhe rreshtit gjejme se ne cilin box jemi
